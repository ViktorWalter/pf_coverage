...
//2.0944
public:
    Controller() : nh_priv_("~"), fow_controller(2.79, SAFETY_DIST, ROBOT_RANGE, ROBOTS_NUM-1), hqp_solver(2.79, SAFETY_DIST, ROBOT_RANGE, ROBOTS_NUM-1)
    {
        //------------------------------------------------- ROS parameters ---------------------------------------------------------
    ...
    timer_ = nh_.createTimer(ros::Duration(0.1), std::bind(&Controller::cbf_coverage2, this));
...
void Controller::cbf_coverage2()
{
    ...
    {
        // slack.col(i) =  slack_max.cwiseProduct(sigmoid(2*(distances[i] - 2*SAFETY_DIST)) * Eigen::VectorXd::Ones(4)) + slack_neg.col(i);
        slack.col(i) =  slack_max.cwiseProduct(std::tanh(distances[i] - dist_lim) * Eigen::VectorXd::Ones(4)); // + slack_neg.col(i);
    }
